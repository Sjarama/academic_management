...
BASE_URL = os.getenv('BASE_URL', "http://localhost:5501")
...
def start_driver():
    opts = Options()
    # Use the newer headless flag for recent Chrome versions and add common flags
    # Allow overriding headless mode with env var `HEADLESS=0`
    headless_env = os.getenv('HEADLESS', '1')
    if headless_env not in ('0', 'false', 'False'):
        try:
            opts.add_argument('--headless=new')
        except Exception:
            opts.headless = True
    # Common flags for CI/Windows environments
    ...
    # Window size already set via option; ensure it's applied
    return driver
