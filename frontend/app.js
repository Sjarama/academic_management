...
async function renderDashboard() {
  ...
  renderFailingCoursesBarChart(validCourses);
}
...
function renderFailingCoursesBarChart(courses) {
  ...
  const entries = courses
    .map(course => ({
      ...
      teacherId: course.teacherId != null ? course.teacherId : "N/A",
...
function drawBarChart(ctx, canvas, legend, infoBox, state) {
    ...
    entries.forEach((entry, index) => {
      ...
      legendItem.innerHTML = `
        ...
        <div class="legend-meta">
          ...
          <span>Profesor ID: ${entry.teacherId}</span>
        </div>
      `;
  ...
  if (activeIndex != null) {
    ...
    infoBox.textContent = `${entry.label} · Profesor ${entry.teacherId} · ${entry.percentage}% reprobación`;
  }
}
